...
function RecipeCard({ recipe, onOpen, onFavourite, index }: { recipe: Recipe; onOpen: () => void; onFavourite: () => void; index: number }) {
  ...
  return <article role="button" tabIndex={0} onKeyDown={onKey} onClick={onOpen} className="polaroid-card" style={{ rotate: `	ext{[-1.4, 1.1, -0.6, 1.6][index % 4]}deg` }} aria-label={`Open 	ext{recipe.title}`}><RecipeImage recipe={recipe} /><div className="mt-4 flex items-start justify-between gap-3"><div><h3 className="text-xl font-black">{recipe.title}</h3><p className="text-sm text-[var(--mutedInk)]">{recipe.subtitle}</p></div><button className="heart-button" onClick={(event) => { event.stopPropagation(); onFavourite(); }} aria-label={`	ext{recipe.favourite ? "Remove" : "Add"} 	ext{recipe.title} favourite`}>{recipe.favourite ? "♥" : "♡"}</button></div><div className="mt-3 flex flex-wrap gap-2">{recipe.tags.map((tag) => <span className="tag" key={tag}>{tag}</span>)}</div><p className="mt-3 text-sm text-[var(--mutedInk)]">{icons.clock} Prep {recipe.prepTime} · Cook {recipe.cookTime}</p></article>;
}
...
export default function App() {
  ...
  if (view === "activeCook") return <ActiveCookMode recipe={selectedRecipe} stepIndex={activeStepIndex} checked={checkedIngredients} setStep={setActiveStepIndex} onExit={() => setView("detail")} toggleIngredient={(index) => setCheckedIngredients((current) => { const next = new Set(current); next.has(index) ? next.delete(index) : next.add(index); return next; })} />;
