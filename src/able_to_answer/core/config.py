...
class Settings:
    ...
    def __post_init__(self) -> None:
        if self.chunk_size_chars <= 0:
            raise ValueError("ATA_CHUNK_SIZE_CHARS must be greater than 0")
        if self.chunk_overlap_chars < 0:
            raise ValueError("ATA_CHUNK_OVERLAP_CHARS must be greater than or equal to 0")
        if self.chunk_overlap_chars >= self.chunk_size_chars:
            raise ValueError(
                "ATA_CHUNK_OVERLAP_CHARS must be strictly less than "
                "ATA_CHUNK_SIZE_CHARS to avoid an infinite loop while chunking"
            )
        if self.max_context_chunks <= 0:
            raise ValueError("ATA_MAX_CONTEXT_CHUNKS must be greater than 0")
        if self.max_answer_chars <= 0:
            raise ValueError("ATA_MAX_ANSWER_CHARS must be greater than 0")
